...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        proc_first = scheme_eval(first, env)
        validate_procedure(proc_first)
        if isinstance(proc_first, MacroProcedure):
            return scheme_eval(proc_first.apply_macro(rest, env), env)
        args_rest = rest.map(lambda r: scheme_eval(r, env))
        return scheme_apply(proc_first, args_rest, env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Error: Cannot apply arguments {} to formals {}".format(
                vals, formals))
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError(
                'Wrong number of arguments provided: {0}'.format(python_args))
        # END PROBLEM 3
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        names, values = Pair(val.first, names), Pair(
            scheme_eval(val.rest.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    validate_form(
        expressions, 2)  # Checks that expressions is a list of length at least 2
    target = expressions.first  # (f x y)
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        formals = target.rest
        validate_formals(formals)
        env.define(target.first, MacroProcedure(
            formals, expressions.rest, env))
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('Improper Form {0}'.format(bad_target))
    # END Problem 20
